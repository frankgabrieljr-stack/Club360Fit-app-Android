...
struct ClientNotificationDTO: Decodable, Sendable, Identifiable {
    ...
    /// Stable across renders — never use `UUID()` here or `List` / `ForEach` diffing breaks (UICollectionView assert).
    var id: String {
        if let rowId, !rowId.isEmpty { return rowId }
        return "\(clientId)|\(createdAt ?? "")|\(title)|\(body)"
    }
