...
struct MyScheduleView: View {
    ...
    @State private var expandedSessionHelp: Set<String> = []
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Club360InfoSectionHeader(
                        ...
                        helpTitle: nil,
                        helpBody:
                            "Your coach schedules club sessions and one-on-ones here. Upcoming shows what’s still ahead; "
                            + "Past lists earlier sessions. Notes appear when your coach adds them.",
                    ...
                    if !model.upcoming.isEmpty {
                        ...
                        ForEach(model.upcoming, id: \.id) { s in
                            sessionBlock(s, small: false, sessionHelpExpanded: $expandedSessionHelp)
                        }
                    }
                    if !model.past.isEmpty {
                        ...
                        ForEach(Array(model.past.suffix(20)), id: \.id) { s in
                            sessionBlock(s, small: true, sessionHelpExpanded: $expandedSessionHelp)
                        }
                    }
                }
                .padding()
    ...
    private func sessionBlock(
        _ s: ScheduleEventDTO,
        small: Bool,
        sessionHelpExpanded: Binding<Set<String>>
    ) -> some View {
        let expanded = Binding<Bool>(
            get: { sessionHelpExpanded.wrappedValue.contains(s.id) },
            set: { newValue in
                if newValue {
                    sessionHelpExpanded.wrappedValue.insert(s.id)
                } else {
                    sessionHelpExpanded.wrappedValue.remove(s.id)
                }
            }
        )
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if sessionHelpExpanded.wrappedValue.contains(s.id) {
                Club360InfoHelpBlock(
                    helpTitle: "Session",
                    helpBody:
                        "Date and time are set by your coach. Notes are optional reminders or details for that visit."
                )
            }
