...
struct MyPaymentsView: View {
    ...
    @State private var showVenmoHelp = false
                        ...
                        if let note = s.note?.trimmingCharacters(in: .whitespacesAndNewlines), !note.isEmpty {
                            VStack(alignment: .leading, spacing: 12) {
                                Club360InfoSectionHeader(
                                    ...
                                    helpTitle: "Coach note",
                                    helpBody:
                                        "This message is from your coach about billing or payment expectations. "
                                        + "It isn’t a receipt—use Venmo/Zelle and confirmations below for status.",
                                    isExpanded: $showCoachNoteHelp
                                )
                                Text(note)
                                    .font(.body)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack(alignment: .top) {
                        Text("Upcoming due")
                            ...
                            .foregroundStyle(Club360Theme.cardTitle)
                        Spacer(minLength: 8)
                        Club360InfoTrailingButton(isExpanded: $showUpcomingDueHelp)
                    }
                    if showUpcomingDueHelp {
                        Club360InfoHelpBlock(
                            helpTitle: "What’s due next",
                            helpBody:
                                "Your coach sets the next due date, amount, and any short note. "
                                + "Pay through Venmo or Zelle using the details below, then tap I paid."
                        )
                    }
    ...
    private func confirmationCard(
        _ c: PaymentConfirmationDTO,
        expandedConfirmationHelp: Binding<Set<String>>
    ) -> some View {
        let statusLabel = confirmationStatusLabel(c.status)
        let expanded = Binding<Bool>(
            get: { expandedConfirmationHelp.wrappedValue.contains(c.id) },
            set: { newValue in
                if newValue {
                    expandedConfirmationHelp.wrappedValue.insert(c.id)
                } else {
                    expandedConfirmationHelp.wrappedValue.remove(c.id)
                }
            }
        )
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if expandedConfirmationHelp.wrappedValue.contains(c.id) {
                Club360InfoHelpBlock(
                    ...
                    helpBody:
                        "You submitted this after paying. Pending means your coach hasn’t verified yet; "
                        + "Verified means they matched it. Declined means they couldn’t confirm—reach out to them."
                )
            }
    ...
    private func recordCard(_ r: PaymentRecordDTO, expandedRecordHelp: Binding<Set<String>>) -> some View {
        let expanded = Binding<Bool>(
            get: { expandedRecordHelp.wrappedValue.contains(r.id) },
            set: { newValue in
                if newValue {
                    expandedRecordHelp.wrappedValue.insert(r.id)
                } else {
                    expandedRecordHelp.wrappedValue.remove(r.id)
                }
            }
        )
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if expandedRecordHelp.wrappedValue.contains(r.id) {
                Club360InfoHelpBlock(
                    ...
                    helpBody:
                        "These entries are logged by your coach when they record a received payment. "
                        + "They’re for your reference alongside your own confirmations above."
                )
            }
