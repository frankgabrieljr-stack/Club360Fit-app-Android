...
struct MyProgressView: View {
    ...
    @State private var expandedCheckInHelp: Set<String> = []
                    ...
                    if model.checkIns.isEmpty, !model.isLoading {
                        ...
                        ForEach(model.checkIns, id: \.rowIdentity) { checkIn in
                            let checkExpanded = Binding<Bool>(
                                get: { expandedCheckInHelp.contains(checkIn.rowIdentity) },
                                set: { newValue in
                                    if newValue {
                                        expandedCheckInHelp.insert(checkIn.rowIdentity)
                                    } else {
                                        expandedCheckInHelp.remove(checkIn.rowIdentity)
                                    }
                                }
                            )
                            VStack(alignment: .leading, spacing: 8) {
                                HStack(alignment: .top, spacing: 10) {
                                    ...
                                    Spacer(minLength: 8)
                                    Club360InfoTrailingButton(isExpanded: checkExpanded)
                                }
                                if expandedCheckInHelp.contains(checkIn.rowIdentity) {
                                    Club360InfoHelpBlock(
                                        helpTitle: "Check-in",
                                        helpBody:
                                            "Weight is optional and stored privately for your coach. "
                                            + "Workout and meals flags help them see consistency at a glance."
                                    )
                                }
...
private struct LogProgressSheet: View {
    ...
    private func save() async {
        ...
        let weightLbs = wTrim.isEmpty ? nil : Double(wTrim)
        if !wTrim.isEmpty, weightLbs == nil {
            errorMessage = "Enter a valid weight in pounds."
            return
        }
        let weightKg: Double? = weightLbs.map { Club360Units.kgFromPounds($0) }
